...
class AuthController extends Controller
{
    public function register(Request $request)
    {
        try {
            ...
            return response()->json([
                ...
                'user_id' => $user->id,
                // 'otp_debug' => $otp  // Remove debug in prod-like environment
            ]);
        } catch (\Exception $e) {
    ...
    public function login(Request $request)
    {
        try {
            ...
            if (!$user->last_login_at || Carbon::parse($user->last_login_at)->lt($twoMonthsAgo)) {
                 ...
                 return response()->json([
                     'message' => 'OTP Required. Sent to email.',
                     'require_otp' => true,
                     'user_id' => $user->id
                 ]);
            }
    ...
    public function verifyOtp(Request $request)
    {
        try {
            ...
            return response()->json(['error' => 'OTP Verification failed', 'message' => $e->getMessage()], 500);
        }
    }
